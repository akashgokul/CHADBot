...
def edge_detect_naive(gray_img):
    ...
    # np.vstack((gray_img,np.zeroes(gray_img.size)))
    padded = np.zeros((gray_img.shape[0]+2,gray_img.shape[1]+2))

    padded[1:gray_img_blur.shape[0]+1,1:gray_img_blur.shape[1]+1] = gray_img_blur
    
    # print(gray_img_blur.shape)
    # print(padded.shape)
    # print(padded)

    gx = ndimage.convolve(padded,kx)
    gy = ndimage.convolve(padded,ky)
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    img_r = img_d[:]
    img_r.shape = (img_d.shape[0]*img_d.shape[1], img_d.shape[2])
    # img_r = np.shape(img_d.shape[0]*img_d.shape[1], img_d[2])
